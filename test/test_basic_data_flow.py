...
input_node = Node(
    ...
    config=InputNodeConfig(data_provider=lambda : [{"value": 1}, {"value": 3}, {"value": 5}, {"value": 7}, {"value": 9}])
)
...
filter_node = Node(
    ...
    config=FilterNodeConfig(filter_handler=lambda item: item['value'] > 5)
)
...
def main():
    result_graph = asyncio.run(test())

    print("过滤后的数据：", result_graph.get_node_by_id(filter_node.id).result.get_result_data())
