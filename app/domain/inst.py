...
from app.domain.storage_info import StorageInfo
...
from app.utils.dot_notation_converter import dotter
...
class NullableInst(Creatable):
    id: Optional[str] = None


class NullablePrimaryInst(NullableInst):
    primary_id: Optional[str] = None


class Inst(Creatable):
    id: str
    _metadata: Optional[RecordMetadata] = PrivateAttr(None)

    def set_meta_data(self, meta_data: Optional[RecordMetadata] = None) -> 'Inst':
    ...
    def to_storage_record(self, exclude=None, exclude_unset: bool = False) -> StorageRecord:
        ...
        if self._metadata:
            ...
            storage_info = self.storage_info()  # type: Operational[StorageInfo]
            if storage_info and storage_info.multi is True:
                if isinstance(self, Operational):
                    if isinstance(self.operation.new, bool) and not self.operation.new:
                        logger.error(
                            f"Inst {type(self)} does not have index set. And it is not new.",
                            extra=ExtraInfo.build(object_=self, origin="storage", error_number="S0001")
                        )
                else:
                    logger.warning(
                        f"Inst {type(self)} converts to index-less storage record.",
                        extra=ExtraInfo.build(object_=self, origin="storage", error_number="S0002")
                    )
        return record

    @staticmethod
    def new() -> 'Inst':
        return Inst(id=str(uuid4()))
    ...
    def __eq__(self, other):
        return self.id == other.id if isinstance(other, Inst) else False
...
class FlatInst(Dotty):
    ...
    def monitor_changes(self, flag: bool) -> 'FlatInst':
        if flag:
    ...
    def set_meta_data(self, metadata: RecordMetadata = None) -> 'FlatInst':
...
@contextmanager
def change_monitor(flat_inst: FlatInst):
    flat_inst.monitor_changes(True)
    yield
    flat_inst.monitor_changes(False)
