...
from typing import List, Union, Tuple, Optional, Dict, AsyncIterable, Any

from pydantic import BaseModel, ValidationError

from app.exception.dag_error import DagExecError
from app.service.data_flow.domain.debug_info import DebugInfo, DebugNodeInfo
...
from app.service.plugin.domain.result import VoidResult, Result, MissingResult
from app.service.plugin.runner import ActionRunner
from app.service.value_threshold_manager import ValueThresholdManager
from app.utils import extra_info as ExtraInfo
from app.domain.enum_value import EventStatus
from app.utils.console import ConsoleStatus
...
import app.service.data_flow.plugin.life_cycle as LifeCycle
from app.utils.excetions import get_traceback
from app.service.data_flow.domain.tasks_results import ActionsResults
...
class InputEdges:
    ...
    def add_edge(self, id_: str, edge: InputEdge):
        self.edges[id_] = edge

    def has_active_edges(self) -> bool:
        for _, edge in self.edges.items():
            if isinstance(edge, bool) and edge.active:
                return True
        return False

    def has_edge(self, id_) -> bool:
        return self.edges[id_] if id_ in self.edges else False

    def get_first_edge(self) -> Optional[InputEdge]:
        edge_ids = list(self.edges.keys())
        if edge_ids:
            return self.edges[edge_ids[0]]
        return None
...
class GraphInvoker(BaseModel):
    ...
    @staticmethod
    def _add_to_event_loop(tasks: List[Tuple[asyncio.Task, str, Any, Edge, bool]],
                           coroutine, port: str, params, edge: Edge, active: bool):
    ...
    async def _run_in_event_loop(self, tasks, node: Node, params, _port, _task_result, in_edge: Edge = None):
        try:
            ...
            if isinstance(node.skip, bool) and node.skip:
                ...
                if isinstance(node.run_once.enable, bool) and \
                    ...
                    migration_data = {
                        ...
                        "payload": list(params.values()),
                        "event": node.object.event,
                        "flow": node.object.flow,
                        "memory": node.object.memory
                    }
                    visitor = DataVisitor(migration_data)
                    if node.run_once.type == "value":
                        value = visitor[node.run_once.value]
                    elif node.run_once.type == "condition":
                        value = None
                    else:
                        raise ValueError(f"Unknown type {node.run_once.type} for conditional workflow stop.")
                    if not await vtm.pass_threshold(value):
                        coroutine = self._void_return(node)
                        node.object.console.log(f"Node '{node.name}' stopped; Condition value '{value}' has not changed.")
                    else:
                        coroutine = LifeCycle.plugin.execute(node, params)
                        node.object.console.log(
                            f"Node '{node.name}' executed; Conditional value '{value}' has changed."
                        )
                else:
                    coroutine = LifeCycle.plugin.execute(node, params)
            ...
            arg_spec = inspect.getfullargspec(node.object.run)
            method_params = arg_spec.args[1:]
            if not set(method_params).intersection(set(params.keys())):
                raise DagExecError(
                    f"Misconfiguration of node type '{node.class_name}'. Method 'run' expects input parameters [{', '.join(method_params)}], "
                    f"but received [{', '.join(params.keys())}]. TypeError: '{str(e)}'",
                    traceback=get_traceback(e)
                )

    def set_sessions(self, session: Session):
    ...
    async def run_node(self, node: Node, payload, ready_upstream_results: ActionsResults) -> AsyncIterable[Tuple[
        Result, float, Optional[Session], ConsoleStatus, InputEdges]]:
        ...
        if node.start:
            ...
            for start_port, edge, end_port in node.graph.in_edges.get_enabled_edges():  # type: str, Edge, str
                ...
                for upstream_result in ready_upstream_results.get(
                    edge.id, start_port):  # type: Union[Result]
                    if isinstance(upstream_result, MissingResult):
                        ...
                        params = {
                            end_port: upstream_result_copy.value
                        }
        ...
        for task, input_port, input_params, edge, active in tasks:
            ...
            try:
                ...
                _current_session_reference = node.object.session
                if not isinstance(_current_session_reference, Session) or _current_session_reference is _prev_session_reference:
            ...
            for out_port, out_payload in joined_output_results.items():
                if node.object.join.has_reshape_templates():
                    ...
                    if output:
                        ...
                        temp_value = node.object.join.get_reshape_template(out_port).default
                        if isinstance(temp_value, bool) and temp_value:
    ...
    async def init(self, debug_info: DebugInfo, log_list: List[Log], flow, flow_history, event, session,
                   ux: list):
        metrics = {}
        memory = {}
            ...
            try:
                if node.remote is True and not node.is_microservice_configured():
                    raise RuntimeError("Microservice not configured. This node is a remote node but the remote "
                                       ...
                                       "in node settings.")
                node.object = await LifeCycle.plugin.create_instance(node)
                node.object = LifeCycle.plugin.set_context(
                    node,
                    event,
                    session,
                    flow,
    ...
    def is_in_debug_mode(self, event: Event):
        """
    ...
    async def run(self,
                  ...
                  log_list: List[Log],
                  ) -> Tuple[DebugInfo, List[Log], Session, Event]:
