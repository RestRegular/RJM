...
from app.utils.yaml_config import YamlConfig
...
from app.utils.environment import get_env_as_int, get_env_as_bool
...
class MysqlConfig:
    def __init__(self, env):
        ...
        self.mysql_database = env.get('MYSQL_DATABASE', "rjm")
...
class RJMConfig(metaclass=Singleton):
    def __init__(self, env):
        self.env = env
        ...
        self.auto_profile_merging = env.get('AUTO_PROFILE_MERGING', 's>a.d-kljsa87^5adhcs')
        ...
        if self.multi_tenant and (self.multi_tenant_manager_url is None or self.multi_tenant_manager_api_key is None):
            if self.multi_tenant_manager_url is None:
                logger.warning('No MULTI_TENANT_MANAGER_URL set for MULTI_TENANT mode. Either set '
                               ...
                               extra=ExtraInfo.build(object_=self, origin="configuration", error_number="C0001")
                               )
            ...
            if self.multi_tenant_manager_api_key is None:
                logger.warning('No MULTI_TENANT_MANAGER_API_KEY set for MULTI_TENANT mode. Either set '
                               ...
                               extra=ExtraInfo.build(object_=self, origin="configuration", error_number="C0002")
                               )
        ...
        if self.multi_tenant and not is_valid_url(self.multi_tenant_manager_url):
            logger.warning('Env MULTI_TENANT_MANAGER_URL is not valid URL.',
                           extra=ExtraInfo.build(object_=self, origin="configuration", error_number="C0003")
                           )
        ...
        if self.apm_on and self.auto_profile_merging and len(self.auto_profile_merging) < 20:
            logger.warning(
                ...
                extra=ExtraInfo.build(object_=self, origin="configuration", error_number="C0004")
            )
