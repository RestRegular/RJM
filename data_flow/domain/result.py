...
class ExecutionResult(Result, ABC):
    ...
    result_data: Dict[str, Any] = Field(default_factory=dict, description="输出数据，键为端口ID")
...
class DefaultExecutionResult(ExecutionResult):
    """默认执行结果实现类"""
...
class BatchExecutionResult(ExecutionResult):
...
def test():
    # 创建普通执行结果
    result = DefaultExecutionResult(node_id="filter_node_123")
    try:
        # 执行节点逻辑
        filtered_data = [1, 2, 3]
        result.set_output("passed", filtered_data)
        result.mark_success()
    ...
    # 创建批处理执行结果
    batch_result = BatchExecutionResult(node_id="batch_processor_456")
