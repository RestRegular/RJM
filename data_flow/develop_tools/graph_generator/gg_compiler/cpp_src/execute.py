...
from newrcc.c_color import TextColor, Decoration, RESET
...
def log(
    ...
    """打印带颜色的日志信息"""
    print(f"{start}{COMMAND_OUTPUT_PREFIX} {ctext(msg, color, decorations=decorations)}", end=end)
...
def error(
    ...
    """打印错误信息并退出程序"""
    print(f"{start}{COMMAND_OUTPUT_PREFIX} {ctext(msg, TextColor.RED, decorations=decorations)}", end=end)
    sys.exit(exit_code)
