...
class ConsoleHandler(logging.Handler):
    ...
    def format(self, record):
        ...
        if self.colorful_log:
            ...
            time_str = ctext(time_str, TextColor((146, 146, 215)))
            level_name = ctext(level_name, level_color, decorations=[Decoration.BOLD])
            log_msg = ctext(log_msg, level_color)
            module_name = ctext(module_name, TextColor.PURPLE, decorations=[Decoration.BOLD])
            location = ctext(location, TextColor.CYAN, decorations=[Decoration.ITALIC, Decoration.L_UNDERLINE])
    ...
    def format_exception(self, exc_info) -> str:
        ...
        result = ''.join([ctext(line, TextColor.RED) if self.colorful_log else line for line in traceback.format_exception(error)])
        return result
...
def get_logger(
        name: str,
        log_level: Union[str, Literal['notset', 'debug', 'info', 'warn', 'warning', 'error', 'fatal', 'critical']] = 'notset',
        to_console: bool = True,
        log_file: Optional[str] = None,
        colorful_log: bool = True
) -> logging.Logger:
    ...
    if isinstance(logger, LoggerPro):
