...
class RJMatchDegreeCalculator:
    ...
    def _calculate_title_match(self, resume: dict, job: dict) -> float:
        ...
        Args:
            ...
            job: 岗位数据字典（需包含 'jobTitle' 字段）

        Returns:
            职位名称匹配度（0-1之间）
        """
        resume_title = resume.get('title', '')  # 简历中的期望职位
        job_title = job.get('jobTitle', '')  # 岗位名称
    ...
    def _calculate_work_experience_match(self, resume: dict, job: dict) -> float:
        ...
        Args:
            resume: 简历数据字典（需包含'workExperience'字段，工作经历列表）
            job: 岗位数据字典（需包含'minimumWorkTime'字段，最低工作时间要求）
        ...
        # 提取岗位要求的最低工作年限
        min_work_time = job.get('minimumWorkTime', '')
    ...
    def _calculate_salary_match(self, resume: dict, job: dict) -> float:
        ...
        Args:
            ...
            job: 岗位数据字典（需包含 'salaryRange' 字段，岗位薪资范围）
        ...
        job_salary = job.get('salaryRange', '')  # 岗位薪资范围
    ...
    def _calculate_project_experience_match(self, resume: dict, job: dict) -> float:
        ...
        Args:
            resume: 简历数据字典（需包含'projectExperience'字段，项目经历列表）
            job: 岗位数据字典（需包含'jobDescription'字段，岗位描述文本）

        Returns:
            项目经历匹配度（0-1之间）；若无项目经历返回0.3，无岗位描述返回0.5
        """
        projects = resume.get('projectExperience', [])  # 项目经历列表
        job_description = job.get('jobDescription', '')  # 岗位描述文本
    ...
    def _calculate_other_factors(self, resume: dict, job: dict) -> float:
        ...
        # 提取岗位描述关键词
        job_desc = job.get('jobDescription', '')
...
if __name__ == "__main__":
    ...
    sample_job = {
        ...
        "province": "北京市",
        "salaryRange": "18k-25k",
        "jobDescription": "负责公司产品前端开发与维护，参与需求分析和技术方案设计",
